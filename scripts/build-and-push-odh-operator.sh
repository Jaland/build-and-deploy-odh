...
if [[ "${SKIP_GET_MANIFESTS}" != "1" ]]; then
  ...
  fi
  {
    ...
    echo "# The fetch uses --maas below when MAAS_OVERRIDE is set; the copied get_all_manifests.sh may still"
    echo "# show upstream [\"maas\"]= unless MAAS_MANIFEST_WRITE_FILE=1."
